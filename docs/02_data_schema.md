...
- 標籤清單維護於 `docs/06_tags_taxonomy.md`
- **2026-04-29 上限規則**:每節點 ≤ 4 個 tag(超量會觸發 W8 警告)。三層結構:
  1. **支出類型詞**(必填,1-2 個):確保 `EXPENSE_LAYER` 推斷正確,例:`交通費` / `住宿費` / `生活費` / `保險費`
  2. **操作特徵詞**(選填):當特徵是節點主題時才加,例:`報支上限` / `覈實報支` / `電子憑證`
  3. **身分/場景限定詞**(選填):明確規範特定身分/場景時才加,例:`國外專家` / `校務基金` / `偏遠地區`
- **避免**:`程序` / `規定` / `辦法` / `處理` 等無區辨力虛詞;母題名稱不重複當 tag(已由 `EXPENSE_LAYER` 處理)

### 1.4 `status` 4 值制(2026-04-29 起)

| status 值 | 語意 | 前端行為 |
|-----------|------|----------|
| `現行` | 現行有效 | 預設值,正面顯示 |
| `被取代` | 已被新函釋/修正令取代 | 卡片橘色「被取代」徽章,**強制配 `superseded_by` 欄**指向現行節點 |
| `修正中` | 修正草案或待確認 | 卡片黃色「修正中」徽章,加 hint 提示 |
| `已廢止` | 已廢止 | 預設隱藏(`HIDE_OBSOLETE = true`),除非有 `effective_period` 例外保留 |

**替換鏈規則**:`status: 被取代` 節點必須在 `superseded_by` 欄填現行替代節點 ID,形成可追蹤的替換鏈,確保查詢回傳優先序「最新法規(最優先)→ 函釋(次)→ Q&A(末)」。

### 1.5 `certainty` + `disclaimer_level` 信度系統(Phase 4 標記中)

| certainty | 語意 | 預設 disclaimer_level |
|-----------|------|----------------------|
| `explicit` | 法規或函釋有明文規定 | `standard` |
| `inferred` | 依法規精神推論,無明文 | `strong` |
| `contested` | 實務有爭議或各機關解釋不一 | `strong` |
