...
// 依賴: ../static/js/allowance-data.js (CITY_TREE, CITY_RATES, getDailyRate, buildCityOptions, onCitySearch)
// 不汙染 P01 主模組：所有 DOM 操作限定在 #view-report 內
...
function toChineseAmount(num) {
    ...
    function convertGroup(g) {
        if (g === 0) return '';
        const d4 = [Math.floor(g / 1000), Math.floor((g % 1000) / 100),
                    Math.floor((g % 100) / 10), g % 10];
        const u4 = ['仟', '佰', '拾', ''];
        let s = '', prevZero = false;
        for (let i = 0; i < 4; i++) {
            if (d4[i] === 0) { if (s.length > 0) prevZero = true; }
            else { if (prevZero) { s += '零'; prevZero = false; } s += digits[d4[i]] + u4[i]; }
        }
        return s;
    }
    ...
    for (let i = 0; i < 3; i++) {
        if (groups[i] === 0) { if (result.length > 0) prevWasZeroGroup = true; continue; }
        if (result.length > 0 && (prevWasZeroGroup || groups[i] < 1000)) result += '零';
        prevWasZeroGroup = false;
        result += convertGroup(groups[i]) + groupUnits[2 - i];
    }
    return result + '元整';
}
...
function sumField(entries, field) {
    return entries.reduce((s, e) => s + parseCurrency(e[field] || 0), 0);
}

const EXPENSE_FIELDS = ['airfare', 'ship', 'ground', 'living', 'handling', 'insurance', 'admin', 'gifts'];

function dayTotal(entry) {
    return EXPENSE_FIELDS.reduce((s, f) => s + parseCurrency(entry[f] || 0), 0);
}

function pageTotal(entries) {
    return entries.reduce((s, e) => s + dayTotal(e), 0);
}
...
function loadFromStorage() {
    try {
        ...
        if (saved) formData = Object.assign({}, formData, JSON.parse(saved));
    } catch (e) { /* ignore */ }
}
...
function rebuildEntries(startDate, endDate) {
    ...
    formData.dailyEntries = dates.map((dateStr, i) => {
        ...
        base.month = String(d.getMonth() + 1);
        base.day = String(d.getDate());
        base._dateStr = dateStr;
...
function collectStep2() {
    ...
    rows.forEach((row, i) => {
        ...
        ['location', 'workNote', 'airfare', 'ship', 'ground', 'living',
         'handling', 'insurance', 'admin', 'gifts'].forEach(field => {
            const el = row.querySelector(`[data-field="${field}"]`);
            if (el) formData.dailyEntries[i][field] = el.value;
        });
        const cityEl = row.querySelector('[data-field="cityKey"]');
        if (cityEl) formData.dailyEntries[i].cityKey = cityEl.value;
        ['hasLodging','hasBreakfast','hasLunch','hasDinner'].forEach(f => {
            const el = row.querySelector(`[data-field="${f}"]`);
            if (el) formData.dailyEntries[i][f] = el.checked;
        });
        const specialEl = row.querySelector('[data-field="specialDay"]');
        if (specialEl) formData.dailyEntries[i].specialDay = specialEl.value;
        const lockedEl = row.querySelector('[data-field="livingLocked"]');
        if (lockedEl) formData.dailyEntries[i].livingLocked = lockedEl.dataset.locked === 'true';
    });
}
...
function getGrandTotal() {
    return (formData.dailyEntries || []).reduce((s, e) => s + dayTotal(e), 0);
}

function getFieldTotals() {
    const totals = {};
    EXPENSE_FIELDS.forEach(f => { totals[f] = sumField(formData.dailyEntries || [], f); });
    totals.grand = getGrandTotal();
    return totals;
}
...
function validateStep1() {
    ...
    if (rateEl && rateWarn) {
        ...
        if (rateEl.value && (r < 20 || r > 50)) {
            rateWarn.textContent = '⚠ 匯率超出常見範圍（20–50），請確認是否填寫「現金賣出匯率」';
...
function buildDayTable() {
    ...
    const totalDays = formData.dailyEntries.length;
    const container = document.getElementById('dayTableContainer');
    const hasRate = !!(formData.exchangeRate && formData.defaultCityKey);
    ...
    document.getElementById('tripSummaryText').innerHTML =
        `<strong>${formData.name}</strong> 出差
         ...
         <strong style="color:var(--brand);">${totalDays}</strong> 天`;
    ...
    let html = warningBanner + scrollHint + `<div class="rpt-day-table-wrap"><table class="rpt-day-table">
    <thead>
    ...
    formData.dailyEntries.forEach((entry, i) => {
        ...
        const specialTag = isReturn
            ...
            : isAirplane
            ? '<span class="rpt-special-tag rpt-plane-tag" title="機上過夜視為供宿（30%基礎）">機上</span>' : '';
      ...
      <td class="rpt-date-cell">${dateLabel}${specialTag}</td>`;
        ...
        infoFields.forEach(f => {
            const val = entry[f.key] || (f.key === 'workNote' ? '如事由' : '');
            html += `<td><input type="${f.type}" data-field="${f.key}" value="${val}"
              placeholder="${f.label}" oninput="onExpenseInput(this)"></td>`;
        });
        ...
        const bfChecked = entry.hasBreakfast ? 'checked' : '';
        const lnChecked = entry.hasLunch    ? 'checked' : '';
        const dnChecked = entry.hasDinner   ? 'checked' : '';
        html += `<td class="rpt-living-cell rpt-center">
          <label class="rpt-meal-chk"><input type="checkbox" data-field="hasBreakfast" ${bfChecked} onchange="onLivingChange(this)">早</label>
          <label class="rpt-meal-chk"><input type="checkbox" data-field="hasLunch" ${lnChecked} onchange="onLivingChange(this)">午</label>
          <label class="rpt-meal-chk"><input type="checkbox" data-field="hasDinner" ${dnChecked} onchange="onLivingChange(this)">晚</label>
        </td>`;
        ...
        html += `<td class="rpt-living-cell rpt-living-input-cell">
          ...
          <button class="rpt-living-reset-btn" onclick="resetLiving(this)" title="重設為自動計算值"
            style="${locked ? '' : 'display:none'}">🔄</button>
        ...
        const noteTitle = (usd && exRate)
            ? `日支數額$${usd}（行政院規定）× 匯率${exRate} × 折扣係數 = ${noteText}`
            : '未設定城市或匯率';
    ...
    <p class="rpt-page-notice">每頁顯示 7 天，列印時自動分頁。<span style="color:var(--ok);">■</span>綠底=自動計算；<span style="color:var(--warn);">■</span>黃底=手動覆蓋。外方供餐請勾選對應項目。</p>`;
...
function onCitySelectStep1(sel) {
    ...
    if (wrap) {
        const inp = wrap.querySelector('.city-search-inp');
        if (inp) { const city = CITY_RATES[sel.value]; inp.value = city ? city.name : ''; }
    }
    saveToStorage();
}
...
function onCitySelectChange(sel) {
    const wrap = sel.closest('.city-combo-wrap');
    if (wrap) {
        const searchInp = wrap.querySelector('.city-search-inp');
        if (searchInp) { const city = CITY_RATES[sel.value]; searchInp.value = city ? city.name : ''; }
    }
    onLivingChange(sel);
}
...
function toggleAirplane(btn) {
    ...
    if (isOn) {
        entry.specialDay = 'none'; btn.classList.remove('active');
    } else {
        entry.specialDay = 'airplane'; btn.classList.add('active');
        if (lodgingCb) { lodgingCb.checked = true; entry.hasLodging = true; }
    }
    ...
    if (dateCell) {
        ...
        const tag = entry.specialDay === 'return'
            ? '<span class="rpt-special-tag rpt-return-tag">回國</span>'
            : entry.specialDay === 'airplane'
            ? '<span class="rpt-special-tag rpt-plane-tag">機上</span>' : '';
...
function autoCalcLiving(entry, dateStr) {
    ...
    const cityKey = entry.cityKey || formData.defaultCityKey || '';
    if (!cityKey) return 0;
    const usd = getDailyRate(cityKey, dateStr || entry._dateStr || '');
    if (!usd) return 0;
...
function refreshLivingCell(row, idx) {
    ...
    const fullTitle = (usd && exRate) ? `日支數額$${usd}（${CITY_RATES[cityKey]?.name || ''}）× 匯率${exRate} × 折扣 = ${note}` : '未設定城市或匯率';
    if (noteEl) { noteEl.innerHTML = `<span class="rpt-living-note">${note}</span>`; noteEl.title = fullTitle; }
}

function onExpenseInput(el) {
    const row = el.closest('.day-row');
    const idx = parseInt(row.dataset.idx);
    collectRowData(row, idx);
    updateRowTotal(idx);
    saveToStorage();
}

function onLivingChange(el) {
    const row = el.closest('.day-row');
    const idx = parseInt(row.dataset.idx);
    collectRowData(row, idx);
    refreshLivingCell(row, idx);
    updateRowTotal(idx);
    saveToStorage();
}
...
function onLodgingChange(el) {
    ...
    if (!el.checked) {
        ...
        if (isReturn) {
            const ok = confirm('回國當天依行政院規定視為供宿（生活費取 30% 基礎計算）。\n取消後將以未供宿（100%）計算，可能超出規定上限。\n確定取消供宿？');
...
function onLivingManualEdit(el) {
    ...
    const lockedHidden = row.querySelector('[data-field="livingLocked"]');
    if (lockedHidden) lockedHidden.dataset.locked = 'true';
...
function resetLiving(btn) {
    ...
    const lockedHidden = row.querySelector('[data-field="livingLocked"]');
    if (lockedHidden) lockedHidden.dataset.locked = 'false';
...
function collectRowData(row, idx) {
    ...
    const cityEl = row.querySelector('.city-select[data-field="cityKey"]');
    if (cityEl) entry.cityKey = cityEl.value;
    ['hasLodging','hasBreakfast','hasLunch','hasDinner'].forEach(f => {
        const el = row.querySelector(`[data-field="${f}"]`);
        if (el) entry[f] = el.checked;
    });
    const specialEl = row.querySelector('[data-field="specialDay"]');
    if (specialEl) entry.specialDay = specialEl.value;
}
...
function updateAllRowTotals() {
    formData.dailyEntries.forEach((_, i) => updateRowTotal(i));
}
...
function printForm01() {
    collectStep2(); collectStep3();
    const html = generateForm01(formData);
    openPrintWindow(html);
}

function printChecklist() {
    const html = generateChecklist(formData);
    openPrintWindow(html);
}
...
function restoreStep1Fields() {
    const map = {
        accountingNumber: 'accountingNumber', name: 'name', title: 'title',
    ...
    const cb = document.getElementById('usesForeignAirline');
    if (cb) cb.checked = !!formData.usesForeignAirline;
    ...
    if (cityEl) {
        ...
        if (formData.defaultCityKey) {
            ...
            if (wrap) {
                const searchInp = wrap.querySelector('.city-search-inp');
                if (searchInp) {
                    const city = CITY_RATES[formData.defaultCityKey];
                    searchInp.value = city ? city.name : '';
                }
            }
        }
    }
...
const LAYOUT_MAIN_COLS = [
    { cls: 'col-cat', label: '類別欄',       def: 5.5,  min: 3,  max: 12, step: 0.1 },
    { cls: 'col-sub', label: '子類別欄',     def: 9.5,  min: 5,  max: 18, step: 0.1 },
    { cls: 'col-day', label: '日期欄（×7）', def: 10.71, min: 7, max: 15, step: 0.1 },
    { cls: 'col-tot', label: '小計欄',       def: 10,   min: 5,  max: 15, step: 0.1 },
];
const LAYOUT_SIG_COLS = [
    { cls: 'col-rem-lbl', label: '備註∕帳號 標籤欄', def: 7,  min: 4,  max: 18, step: 0.5 },
    { cls: 'col-rem-val', label: '備註∕帳號 內容欄', def: 43, min: 20, max: 58, step: 0.5 },
    { cls: 'col-sig1',    label: '出差人欄',          def: 22, min: 10, max: 45, step: 1 },
    { cls: 'col-sig2',    label: '指導教授欄',        def: 36, min: 15, max: 55, step: 1 },
    { cls: 'col-sig3',    label: '單位主管欄',        def: 42, min: 15, max: 60, step: 1 },
    { cls: 'col-stmp1',   label: '主計室欄',          def: 33, min: 15, max: 55, step: 1 },
    { cls: 'col-stmp2',   label: '主計室主任欄',      def: 33, min: 15, max: 55, step: 1 },
    { cls: 'col-stmp3',   label: '校長∕授權欄',      def: 34, min: 15, max: 60, step: 1 },
];
const LAYOUT_ROWS = [
    { v: '--h-hdr1',        label: '預算科目列',     def: 7,  min: 4,  max: 15 },
    { v: '--h-hdr2',        label: '預算值列',       def: 5,  min: 3,  max: 12 },
    { v: '--h-hdr3',        label: '科目代碼列',     def: 6,  min: 3,  max: 12 },
    { v: '--h-name',        label: '姓名職稱列',     def: 10, min: 6,  max: 20 },
    { v: '--h-reason',      label: '出差事由列',     def: 9,  min: 5,  max: 25 },
    { v: '--h-daterange',   label: '出差日期列',     def: 11, min: 6,  max: 20 },
    { v: '--h-date-info',   label: '月日地點工作列', def: 8,  min: 5,  max: 15 },
    { v: '--h-expense',     label: '費用項目列',     def: 8,  min: 5,  max: 15 },
    { v: '--h-total',       label: '合計列',         def: 8,  min: 5,  max: 15 },
    { v: '--h-remark',      label: '備註帳號列',     def: 17, min: 8,  max: 35 },
    { v: '--h-decl',        label: '申報聲明列',     def: 11, min: 6,  max: 30 },
    { v: '--h-amount',      label: '大寫金額列',     def: 11, min: 6,  max: 20 },
    { v: '--h-sig-label',   label: '簽名標題列',     def: 6,  min: 4,  max: 12 },
    { v: '--h-sig',         label: '簽名列',         def: 14, min: 8,  max: 35 },
    { v: '--h-stamp-label', label: '用印標題列',     def: 5,  min: 3,  max: 10 },
    { v: '--h-stamp',       label: '用印列',         def: 20, min: 10, max: 50 },
];

function generateLayoutPanel() {
    function colSliderRows(cols, prefix) {
        return cols.map((c, i) => `
    <div class="ctrl-row">
      <span class="ctrl-label">${c.label}</span>
      <input type="range" class="ctrl-slider" id="sl-${prefix}${i}" min="${c.min}" max="${c.max}" step="${c.step}" value="${c.def}">
      <span class="ctrl-val" id="sv-${prefix}${i}">${c.def.toFixed(1)}%</span>
    </div>`).join('');
    }
    const mainColRows = colSliderRows(LAYOUT_MAIN_COLS, 'mc');
    const sigColRows  = colSliderRows(LAYOUT_SIG_COLS,  'sc');
    const rowRows = LAYOUT_ROWS.map((r, i) => `
    <div class="ctrl-row">
      <span class="ctrl-label">${r.label}</span>
      <input type="range" class="ctrl-slider" id="sl-r${i}" min="${r.min}" max="${r.max}" step="1" value="${r.def}">
      <span class="ctrl-val" id="sv-r${i}">${r.def}mm</span>
    </div>`).join('');
    const mainColJson = JSON.stringify(LAYOUT_MAIN_COLS);
    const sigColJson  = JSON.stringify(LAYOUT_SIG_COLS);
    const rowJson     = JSON.stringify(LAYOUT_ROWS);
    return `
<div id="panel-toggle" onclick="togglePanel()">⚙<br>版<br>面<br>調<br>整</div>
<div id="layout-panel">
  <div id="panel-header"><span>⚙ 版面調整</span><span style="cursor:pointer;font-size:20px;" onclick="togglePanel()">✕</span></div>
  <div id="panel-body">
    <p class="panel-hint">拖曳滑桿即時預覽，列印時面板自動隱藏</p>
    <div class="panel-sec-title">欄寬 — 費用表</div>${mainColRows}
    <div class="panel-sec-title">欄寬 — 備註∕簽名∕用印</div>${sigColRows}
    <div class="panel-sec-title">列高設定</div>${rowRows}
  </div>
  <div id="panel-footer">
...
<script>
(function(){
  var mainCols=${mainColJson};var sigCols=${sigColJson};var rowCtrls=${rowJson};
  var panel=document.getElementById('layout-panel');var toggle=document.getElementById('panel-toggle');
  window.togglePanel=function(){var open=panel.classList.toggle('open');toggle.classList.toggle('panel-open',open);};
  function bindColSliders(cols,prefix){cols.forEach(function(c,i){var sl=document.getElementById('sl-'+prefix+i);var sv=document.getElementById('sv-'+prefix+i);sl.addEventListener('input',function(){var val=parseFloat(this.value);sv.textContent=val.toFixed(1)+'%';document.querySelectorAll('.'+c.cls).forEach(function(el){el.style.width=val+'%';});});});}
  bindColSliders(mainCols,'mc');bindColSliders(sigCols,'sc');
  rowCtrls.forEach(function(r,i){var sl=document.getElementById('sl-r'+i);var sv=document.getElementById('sv-r'+i);sl.addEventListener('input',function(){var val=parseInt(this.value);sv.textContent=val+'mm';document.documentElement.style.setProperty(r.v,val+'mm');});});
  window.resetLayout=function(){mainCols.forEach(function(c,i){var sl=document.getElementById('sl-mc'+i);sl.value=c.def;sl.dispatchEvent(new Event('input'));});sigCols.forEach(function(c,i){var sl=document.getElementById('sl-sc'+i);sl.value=c.def;sl.dispatchEvent(new Event('input'));});rowCtrls.forEach(function(r,i){var sl=document.getElementById('sl-r'+i);sl.value=r.def;sl.dispatchEvent(new Event('input'));});};
})();
<\/script>`;
}
...
function generateForm01(data) {
    ...
    const grandTotals = {};
    EXPENSE_FIELDS.forEach(f => { grandTotals[f] = sumField(entries, f); });
...
@media print{.print-page{margin:0;padding:0;width:100%;}td,th{font-size:9pt;}#layout-panel,#panel-toggle{display:none!important;}}
#panel-toggle{position:fixed;top:50%;right:0;transform:translateY(-50%);background:#0057a8;color:#fff;width:28px;padding:10px 5px;border-radius:6px 0 0 6px;cursor:pointer;text-align:center;font-size:11px;line-height:1.7;box-shadow:-2px 2px 8px rgba(0,0,0,.25);z-index:10001;transition:right .3s;font-family:-apple-system,'Microsoft JhengHei',sans-serif;user-select:none;}
#panel-toggle.panel-open{right:280px;}
#layout-panel{position:fixed;top:0;right:0;width:280px;height:100vh;background:#fff;border-left:2px solid #0057a8;box-shadow:-4px 0 20px rgba(0,0,0,.18);z-index:10000;font-family:-apple-system,'Microsoft JhengHei',sans-serif;font-size:13px;display:flex;flex-direction:column;transform:translateX(282px);transition:transform .3s ease;}
#layout-panel.open{transform:translateX(0);}
#panel-header{background:#0057a8;color:#fff;padding:11px 14px;font-size:14px;font-weight:bold;display:flex;justify-content:space-between;align-items:center;flex-shrink:0;}
#panel-body{flex:1;overflow-y:auto;padding:10px 14px;}
#panel-footer{padding:10px 14px;border-top:1px solid #e0e4ec;display:flex;gap:8px;flex-shrink:0;}
.panel-hint{font-size:11px;color:#777;background:#f4f6ff;border-radius:4px;padding:6px 8px;margin-bottom:6px;line-height:1.5;}
.panel-sec-title{font-size:11.5px;font-weight:bold;color:#0057a8;border-bottom:1px solid #c8d4ec;padding-bottom:3px;margin:10px 0 7px;}
.ctrl-row{display:flex;align-items:center;gap:6px;margin-bottom:7px;}
.ctrl-label{flex:0 0 82px;font-size:12px;color:#333;}
.ctrl-slider{flex:1;-webkit-appearance:none;height:4px;border-radius:2px;background:#c5d3ec;outline:none;cursor:pointer;}
.ctrl-slider::-webkit-slider-thumb{-webkit-appearance:none;width:15px;height:15px;border-radius:50%;background:#0057a8;cursor:pointer;border:2px solid #fff;box-shadow:0 1px 3px rgba(0,0,0,.3);}
.ctrl-val{flex:0 0 44px;text-align:right;font-size:12px;color:#444;font-family:monospace;}
.btn-reset{flex:1;padding:8px;background:#f0f2f8;border:1px solid #c5cedf;border-radius:5px;cursor:pointer;font-size:13px;color:#444;}
.btn-print{flex:1;padding:8px;background:#0057a8;color:#fff;border:none;border-radius:5px;cursor:pointer;font-size:13px;font-weight:bold;}
.btn-reset:hover{background:#e2e6f5;}.btn-print:hover{background:#00459c;}
</style></head><body>`;
...
function generateOnePage(data, pageDays, pageIdx, totalPages, isLastPage, grandSum, dateRange, decl1, decl2) {
    const N = 7;
    const dayCount = pageDays.length;
    const pageSubtotals = {};
    EXPENSE_FIELDS.forEach(f => { pageSubtotals[f] = sumField(pageDays, f); });
    ...
    function dayCell(dayIdx, field) {
        if (dayIdx >= dayCount) return `<td class="day-val"></td>`;
        const val = parseCurrency(pageDays[dayIdx][field] || 0);
        return `<td class="day-val">${val ? fmt(val) : ''}</td>`;
    }
    function dayCols(field) { let s=''; for(let i=0;i<N;i++) s+=dayCell(i,field); return s; }
    function infoRowDays(field) {
        let s='';
        for(let i=0;i<N;i++){const val=i<dayCount?(pageDays[i][field]||''):'';s+=`<td class="center" style="font-size:9pt;">${val}</td>`;}
        return s;
    }
  ...
  <table style="margin-top:2mm;">
    ...
    <tr class="tr-stamp-label">
      <td class="lbl center">主計室</td><td class="lbl center">主計室主任</td><td class="lbl center">校長（系主任）或其授權代簽人</td>
    </tr>
    <tr class="tr-stamp"><td></td><td></td><td></td></tr>
  </table>` : '';

    const footerNoteHTML = isLastPage
      ? `<p class="footer-note">附註：校長欄位依本校分層負責表授權決行，金額15萬元（含）以下視經費授權二或三層決行。</p>` : '';
...
<div class="print-page">
  ...
  <table>${colgroup}
        ...
        <tr class="tr-hdr3">
          <td class="val" colspan="10" style="font-size:9pt;padding-left:2mm;">510303-7210　國外旅費－獎助學員生</td>
        </tr>
        <tr class="tr-name">
...
function generateChecklist(data) {
    ...
    const items = [
        { no:1, doc:'國外差旅費報告表', ref:'表 01',
          notes:['須有出差人、計畫主持人、單位主管等簽章','免送研發處及人事室會核','報告表免登打系統憑單編號'] },
        { no:2, doc:'國立成功大學「兼任」專案工作人員國內外差假申請單', ref:'表 02',
          notes:['須有出差人、計畫主持人、單位主管等簽章','如係返回後補登，請於「差假申請事後補登說明」欄位填寫說明','如使用線上請假系統，需檢附系統產出含 QR Code 之列印頁面'] },
        { no:3, doc:'國立成功大學因公出國人員搭乘外國籍航空公司班機申請書', ref:'表 03',
          notes:[usesForeignAirline?'【本次出差需填寫】搭乘非華航、長榮、台灣虎航、星宇等國籍航班時須檢附':'搭乘非國籍航班時須檢附（不含轉機段）','中途轉機搭乘外籍航空不適用'],
          highlight: usesForeignAirline },
        { no:4, doc:'國科會補助公文', ref:'—', notes:['國科會核准補助之公文或電子函文'] },
        { no:5, doc:'註冊費 invoice（收據）', ref:'—',
          notes:['請於發票上註記「本張為唯一收據」並簽名','如以信用卡付款，須另附信用卡對帳單'] },
        { no:6, doc:'議程及論文接受函', ref:'—',
          notes:['需含完整會議議程（含場次、時間）','論文接受通知信（acceptance letter）或摘要收錄證明'] },
        { no:7, doc:'電子機票', ref:'—',
          notes:['電子機票訂位紀錄（e-ticket itinerary）','影本需有簽章確認'] },
        { no:8, doc:'機票付款收據', ref:'—',
          notes:['旅行社收據或線上購票收據','影本需有簽章確認'] },
        { no:9, doc:'國科會線上經費結報登錄送出畫面（截圖）', ref:'—',
          notes:['須完成國科會線上系統結報登錄並列印送出畫面截圖','截圖需顯示計畫編號及補助金額'] },
    ];

    const rows = items.map(item => {
        const notesHtml = item.notes.map(n =>
            `<li style="${item.highlight&&n.includes('【')?'color:#c00;font-weight:bold;':''}">${n}</li>`
        ).join('');
        return `
    <tr style="${item.highlight?'background:#fff3f3;':''}">
      <td class="center" style="width:5%;font-weight:bold;">${item.no}</td>
      <td style="width:30%;font-weight:${item.highlight?'bold':'normal'};color:${item.highlight?'#c00':'inherit'};">
        ${item.doc}${item.highlight?'<br><span style="font-size:8pt;">(本次出差必填)</span>':''}
      </td>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('btnBack2')?.addEventListener('click', () => {
        const hasData = formData.dailyEntries.some(e =>
            parseCurrency(e.living || 0) || parseCurrency(e.airfare || 0) ||
            (e.workNote && e.workNote !== '如事由')
        );
        if (hasData) {
            if (!confirm('返回上一步修改日期後，超出新天數範圍的費用資料將遺失。\n確定繼續？')) return;
        }
        showStep(1);
    });
    ...
    document.getElementById('btnReset')?.addEventListener('click', () => {
        ...
        if (confirm(`確定清除「${name}」的出差資料（旅費合計 ${totalStr}）並重新開始？\n\n此操作無法復原。建議先點「匯出備份」再清除。`)) {
            clearStorage();
            location.reload();
        }
    });
    ...
    document.getElementById('btnImportFile')?.addEventListener('change', function () {
        if (this.files[0]) importFormData(this.files[0]);
        this.value = '';
    });
});
