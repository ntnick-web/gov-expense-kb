...
const EXPENSE_LIST = [
  ...
  '訓練費補助',                                       // 教育訓練
  '計畫申請資格', '業務費', '研究設備費', '差旅費', '管理費', // 國科會專章
  '彈性支用額度', '產學合作',                              // 成功大學專章
  '補助與委託計畫', '計畫經費收支', '預算變更流用',         // 農業部專章
  '會計處理', '帳務管理', '結報程序', '財產管理',           // 農業部專章
];
...
const EXPENSE_LAYER = {
  ...
  '國科會專章':     ['計畫申請資格', '業務費', '研究人力費', '研究設備費', '差旅費', '管理費', '補助項目支用', '採購', '彈性支用額度', '程序與通則'],
  '成功大學專章':   ['彈性支用額度', '產學合作', '程序與通則'],
  '餐費及其他支出': ['膳食費', '茶水費', '財物領用', '財物保管', '財物盤點', '財物報廢', '補助支出', '特支費', '罰款賠償', '通信費', '程序與通則'],
  '採購及履約':     ['採購程序', '採購憑證', '履約管理', '程序與通則'],
  '教育訓練':       ['訓練費補助', '程序與通則'],
  '農業部專章':     ['補助與委託計畫', '計畫經費收支', '預算變更流用', '會計處理', '帳務管理', '支出憑證', '結報程序', '財產管理', '費率基準表', '程序與通則'],
};
...
const PARENT_SORT_IDX = Object.fromEntries(
  ['國內旅費','國外旅費','酬勞費','支出憑證與結報','採購及履約','餐費及其他支出','教育訓練',
   '國外專家','國科會專章','教育部專章','成功大學專章','農業部專章']
  .map((p, i) => [p, i])
);
