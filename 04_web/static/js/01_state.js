...
const DATA_VERSION = '2026-05-10a';
...
// 母題 → 簡稱 (sidebar 用)
const PARENTS = ['國內旅費', '國外旅費', '酬勞費', '支出憑證與結報', '採購及履約', '餐費及其他支出', '教育訓練', '國科會專章', '教育部專章', '成功大學專章', '農業部專章'];
// 整備中母題:chip 顯示為灰色不可點按;卡片與情境全部隱藏
let WIP_PARENTS = new Set(['國科會專章', '餐費及其他支出', '採購及履約', '教育訓練', '教育部專章', '成功大學專章', '農業部專章']);
...
const PARENT_LAW = {
  ...
  '農業部專章':     { full: '農業部主管計畫經費處理作業規定',             short: '農業部計畫經費規定' },
};
// 類別代碼 → art (色點 + label)
